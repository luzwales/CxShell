...
FxShell can be launched by a bastion host, SSO portal, or another local tool using a small Xshell-compatible surface. The request is converted to one in-memory external launch model before it reaches the normal connection workflow.

## Supported forms

```text
FxShell.exe -url ssh://user@server.example:22
FxShell.exe -url sftp://user@server.example:22/home/user
FxShell.exe -l user -p 2222 -pw one-time-password -url ssh://server.example
FxShell.exe -i C:\keys\id_ed25519 -url ssh://user@server.example
FxShell.exe -f C:\Temp\server.xsh
```
...
FxShell intentionally does not implement Xshell remote-command options such as `-e` or `-s`. External links should open a session, not silently execute an arbitrary command on a server.
...
`-f` reads the UTF-16 INI fields from an `.xsh` file: host, port, protocol, username, and private-key path. Xshell's encrypted password is not decrypted. If no usable credential is supplied, FxShell uses its normal in-app authentication prompt.

## Platform notes

- Windows writes `HKCU\Software\Classes\ssh` and `HKCU\Software\Classes\sftp`; administrator permission is not required.
- Linux writes `~/.local/share/applications/fxshell-url-handler.desktop` and uses `xdg-mime` when available.
