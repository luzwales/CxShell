...
public sealed class AgentRuntimeClientTests
{
        ...
        var transport = new RecordingTransport(request =>
        {
            ...
            return method switch
            {
                AgentRuntimeMethodNames.Initialize =>
                    "{\"requestId\":\"initialize-1\",\"ok\":true,\"result\":{\"ok\":true,\"runtime\":\"fxshell-session-gateway\",\"version\":\"0.1\",\"protocol\":\"fxshell-agent\",\"protocolVersion\":\"1\",\"methods\":[],\"capabilities\":[]}}",
                AgentRuntimeMethodNames.RuntimeInfo =>
                    "{\"requestId\":\"runtime-info-1\",\"ok\":true,\"result\":{\"runtime\":\"fxshell-session-gateway\",\"protocol\":\"fxshell-agent\",\"protocolVersion\":\"1\",\"runtimeVersion\":\"0.1\",\"methods\":[],\"capabilities\":[],\"supportedProtocols\":[]}}",
    ...
    {
        var transport = new RecordingTransport(
            "{\"requestId\":\"initialize-legacy\",\"ok\":true,\"result\":{\"ok\":true,\"runtime\":\"fxshell-session-gateway\",\"version\":\"0.1\"}}");
        var client = new AgentRuntimeClient(transport);

        var initialized = await client.InitializeAsync(requestId: "initialize-legacy");

        Assert.Equal("fxshell-session-gateway", initialized.Runtime);
        using var request = JsonDocument.Parse(transport.LastRequest!);
    ...
    public async Task ClientGeneratesRequestIdsWhenCallerOmitsOne()
    {
        ...
        Assert.StartsWith("fxshell-runtime-", response.RequestId, StringComparison.Ordinal);
    }
